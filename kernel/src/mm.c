...
void new_page(page * sb, int cpu, int item_id) {
    assert(sb != NULL);
    int size = (1 << item_id);
    char name[128];
...
void insert_page_to_head (page* sb) {
    int cpu = sb->cpu;
    ...
    //再加在表头之前
    page* listhead = cache_chain[cpu][order];
    // if(cpu_count() == 4) {
    while(!full_page(listhead) && listhead != sb) {
